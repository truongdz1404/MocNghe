...
export default function ProductByCategory({ params }: PageProps) {
    const [category, setCategory] = useState<CategoryDto | null>(null);
    useEffect(() =>{
        const fetchCategory = async () => {
            try {
                const response = await CategoryServices.GetById(parseInt(params.id));
    ...
    <div>
               <div className="bg-gray-50 font-sans">
                   <div className="container mx-auto px-4 py-8">
                    ...
                    <AllProduct categoryId={Number(params.id)} />
                   </div>
               </div>
