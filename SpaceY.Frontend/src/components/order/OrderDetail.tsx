...
import { OrderDto, OrderStatus, OrderDetailDto, UpdateOrderDto } from "@/types/order";
...
interface OrderDetailModalProps {
    ...
    onClose: () => void;
    order: OrderDto | null;
    onOrderUpdated?: () => void; // Callback khi order được cập nhật
}
...
const OrderDetailModal: React.FC<OrderDetailModalProps> = ({ isOpen, onClose, order, onOrderUpdated }) => {
    const [orderDetails, setOrderDetails] = useState<OrderDetailDto[]>([]);
    const [loading, setLoading] = useState(false);
    const [cancelLoading, setCancelLoading] = useState(false);
    const [currentOrder, setCurrentOrder] = useState<OrderDto | null>(order);
    ...
    const handleCancelOrder = async () => {
        ...
        try {
            ...
            const updatedOrder = await OrderServices.UpdateOrderStatus(currentOrder.id, updateOrderDto);

            if (updatedOrder) {
                setCurrentOrder(updatedOrder);
                // Gọi callback để cập nhật danh sách order ở component cha
                if (onOrderUpdated) {
                    onOrderUpdated();
                }
                ...
                >
                    ...
                    <div className="flex items-center justify-between border-b border-gray-200 p-6">
                        <div className="flex flex-col">
                            ...
                            <p className="text-sm text-gray-500 mt-1">
                                Đặt hàng lúc {currentOrder && format(new Date(currentOrder.createdAt), "HH:mm - dd/MM/yyyy")}
                            </p>
                        </div>
                        <button
                            onClick={handleClose}
                                    ...
                                    <div className="p-4">
                                        ...
                                        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium border ${getStatusColor(currentOrder.status)}`}>
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <p className="text-sm text-gray-500">
                                                    {format(new Date(currentOrder.modifiedAt), "dd/MM/yyyy HH:mm")}
                                                </p>
                                            </div>
                                            <div className="space-y-2">
                                                <p className="text-sm font-medium text-gray-700">
                    ...
                    <div className="border-t border-gray-200 p-6">
                        <div className="flex gap-3 justify-end">
                            ...
                            {/* Show Cancel button only if order status is Pending */}
